defmodule WormholeProtocolTest do
  ...
  test "the aggregate can switch from rejecting the past to rewriting it" do
    story = WormholeProtocol.rules_of_reality_story!()

    assert story.rejected ==
             {:error, {:command_arrived_in_the_past, latest_accepted_time: "10:05"}}

    assert story.rewritten == :ok
    assert story.reality_policy == :rewrite_history
    assert story.available_oxygen == -5

    assert Enum.map(story.allocations, & &1.command_id) == [
             "wormhole-1",
             "alloc-1",
             "alloc-2"
           ]
