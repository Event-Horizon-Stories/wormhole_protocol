defmodule WormholeProtocol.ReplayEngine do
  ...
  defp apply_event(%{event: %WormholeProtocol.TimelineOpened{}}, snapshot), do: snapshot

  defp apply_event(%{event: %WormholeProtocol.RealityPolicySet{}}, snapshot), do: snapshot

  defp apply_event(%{event: %WormholeProtocol.SectorRegistered{} = event}, snapshot) do
  ...
  defp apply_event(%{event: %WormholeProtocol.OxygenAllocated{} = event}, snapshot) do
  ...
  defp apply_event(%{event: %WormholeProtocol.ReactorRegistered{} = event}, snapshot) do
  ...
  defp apply_event(%{event: %WormholeProtocol.ReactorShutdown{} = event}, snapshot) do
  ...
  defp apply_event(%{event: %WormholeProtocol.ReactorFailureRecorded{} = event}, snapshot) do
    update_in(snapshot, [:reactors, event.reactor_id], fn reactor ->
